...
impl Tokens {
    ...
    pub fn iter_with_context(&self) -> TokensIterWithContext {
        TokensIterWithContext::new(&self.raw)
    }
...
#[derive(Debug, Clone)]
pub struct TokensIterWithContext<'a> {
    inner: std::slice::Iter<'a, Token>,
    nesting: u32,
}

impl<'a> TokensIterWithContext<'a> {
    fn new(tokens: &'a [Token]) -> TokensIterWithContext<'a> {
        TokensIterWithContext {
            inner: tokens.iter(),
            nesting: 0,
        }
    }
...
impl<'a> Iterator for TokensIterWithContext<'a> {
    type Item = &'a Token;
...
impl FusedIterator for TokensIterWithContext<'_> {}
